...
| `internal/adapter/ai/` | Фасад AI + версионирование 5.x/6.x |
| `internal/adapter/ai/{5_x,6_x}/` | Маппинг domain ↔ OpenAPI types |
...
| `pkg/clientai/{5_x,6_x}/` | **Только** OpenAPI codegen (`backend.gen.go`, `swagger.yaml`) |
...
`internal/adapter/ai.Adapter` реализует монолитный `ClientAi` (~25 методов) и делегирует в `activeClient` (5.x или 6.x). Формальной связи между `ClientAi` и портами use case **нет** — compile-time проверка только через DI.

### Initialize vs InitializeWithRetry

| Метод | Поведение |
|-------|-----------|
| `Initialize` | Выбор 5.x/6.x клиента, проверка версии и лицензии |
...
## AI adapter: версионирование 5.x / 6.x

```mermaid
flowchart LR
    init["Adapter.Initialize"] --> try6["tryInitializeClient6x"]
    try6 -->|fail/skip| reset["BaseClient.Reset()"]
    reset --> try5["tryInitializeClient5x"]
    try6 -->|version 6.x| active["activeClient = 6_x"]
    try5 -->|version 5.x| active2["activeClient = 5_x"]
```

- Поддерживаемый диапазон версий сервера: **5.0.0 ≤ ver < 7.0.0** (`adapter.go`, `init.go`).
- Сначала пробуется 6.x; при неудаче или версии вне 6.x — `BaseClient.Reset()` и fallback на 5.x.
- Skip 6.x логируется через `logInitSkip` → `Debugf` (видно с `-v` или `-l`).
- Оба клиента делят один `client.BaseClient` (JWT, HTTP clients). **Параллельное использование двух клиентов невозможно** — shared mutable state.
- Ручной код маппинга: `internal/adapter/ai/{5_x,6_x}/client.go` (~1000 строк каждый, значительное дублирование).
- Codegen: `pkg/clientai/{5_x,6_x}/` — перегенерация через `go generate` в `gen.go`.

При правках scan/settings/report — проверять **оба** адаптера или выносить shared helpers в `internal/adapter/ai/client/` или новый shared-пакет внутри `adapter/ai/`.
...
- `internal/adapter/ai/client_test.go` — `validateVersion`, `isClient6xVersion`
...
Mockery-моки (~48 файлов) сгенерированы, но почти не используются. При добавлении тестов — приоритет: init flow 6x→5x, exit code mapping, critical use cases.
...
2. **Новый AI-метод:** добавить в `ClientAi` + `Adapter` delegate → реализовать в **обоих** `5_x/client.go` и `6_x/client.go` → добавить в port нужных use cases.
3. **OpenAPI изменился:** обновить `swagger.yaml`, `go generate` в `pkg/clientai/{5_x,6_x}/`, затем adapter mapping.
4. **Не нарушать arch-lint:** use case не должен импортировать adapter или `pkg/clientai`.
5. **Shared logic 5x/6x:** выносить в `internal/adapter/ai/client/` или internal shared helper — главный источник регрессий при дублировании.
